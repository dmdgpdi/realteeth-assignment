...
4.  **언어 규칙 (Language)**: 모든 주석(Comment)과 테스트 설명(Description)은 반드시 **한국어**로 작성하세요.

---

## 🏗 테스트 구조: AAA 패턴 (Arrange-Act-Assert)
모든 테스트 코드는 **AAA 패턴**을 엄격히 준수해야 합니다.
코드 내에 주석으로 단계를 명시하면 가독성이 좋아집니다.

1.  **Arrange (준비)**: 테스트할 객체를 생성하고, 필요한 데이터나 상태를 설정합니다.
2.  **Act (실행)**: 테스트 대상(SUT)의 메서드를 호출하거나 로직을 실행합니다.
3.  **Assert (검증)**: 실행 결과가 기대한 상태와 일치하는지 확인합니다.

### ❌ 나쁜 검증 예 (Weak Assertion)
```typescript
// 단순히 개수만 확인하면, 잘못된 데이터가 들어있어도 테스트가 통과될 수 있음
expect(result.length).toBe(2); 
```

### ⭕ 좋은 검증 예 (Strong Assertion)
```typescript
// 실제 반환된 데이터가 기대한 데이터셋과 정확히 일치하는지 확인.
// 이때, Arrange 단계의 데이터를 가공(slice, map 등)해서 기대값으로 쓰지 말고, 
// 실제 기대하는 구체적인 값(Literal)을 직접 작성하여 비교하세요.
...
```typescript
it("회원 가입 성공 시 유저 정보를 반환해야 한다", async () => {
  // Arrange (준비)
  const userRepo = new InMemoryUserRepository();
  const service = new AuthService(userRepo);
